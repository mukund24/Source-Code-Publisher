...
namespace CStoker
{
  ...
  class CToker
  {
    ...
    string extract(ref string lineRemainder)
    {
      ...
      {
        string temp = extractComment(ref lineRemainder);
 
        // ------------------ remove this after debugging -------------------------- 

        if (temp.IndexOf('\r') > -1)
          throw new Exception("comment has \\r");
        return temp;
      }
    ...
    string extractComment(ref string lineRemainder)
    {
      ...
      if(pos == 0)                          // whole line is C++ comment
      {
        ...
        lineRemainder = lineRemainder.TrimEnd(whiteChars);
